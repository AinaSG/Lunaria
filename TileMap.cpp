...
string TileMap::improvedLevelGenerator(){
  ...
  for (int i = 0; i< MAP_Y; ++i){
    for (int j=0; j<MAP_X; ++j){
      ...
      float valCueva = simCave->valSimplex(i, j)*2;
...
void TileMap::prepareArrays()
{
  ...
  for(int j=0; j<mapSize.y; j++)
  {
    for(int i=0; i<mapSize.x; i++)
    {
      ...
      if(tile != Block::Empty)
      {
        ...
        vertices.push_back(heights[0][0]);

        vertices.push_back(posTile.x + blockSize); vertices.push_back(posTile.y);
        vertices.push_back(texCoordTile[1].x); vertices.push_back(texCoordTile[0].y);
        vertices.push_back(heights[0][1]);

        vertices.push_back(posTile.x + blockSize); vertices.push_back(posTile.y + blockSize);
        vertices.push_back(texCoordTile[1].x); vertices.push_back(texCoordTile[1].y);
        vertices.push_back(heights[1][1]);
        ...
        vertices.push_back(heights[1][0]);
      }
    }
  }
  ...
  glBufferData(GL_ARRAY_BUFFER, 30 * nTiles * sizeof(float), &vertices[0], GL_STATIC_DRAW);
  size_t stride = 5*sizeof(float);
  posLocation = program.bindVertexAttribute("position", 2, stride, 0);
  texCoordLocation = program.bindVertexAttribute("texCoord", 2, stride, (void *)(2*sizeof(float)));
  heightLocation = program.bindVertexAttribute("height",1,stride, (void *)(4*sizeof(float)));
}
